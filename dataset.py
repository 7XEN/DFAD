...
INPUT_DATA = 'spectrogram/A/original'     # 原始输入数据的目录，其有2个子目录，每个目录下保存属于该类别的所有图片
OUTPUT_DATA = 'spectrogram/A/original/data_gry_299.npy'     # 将整理后的图片数据通过numpy的格式保存
...
def create_image_lists(sess, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        # 获取一个子目录中所有的图片文件
        extensions = ['jpg', 'jpeg', 'JPG', 'JPEG', 'png', 'PNG']     # 列出所有扩展名
        file_list = []
    ...
    return np.asarray([training_images, training_labels,
                       ...
                       testing_images, testing_labels], dtype=object)
